<?php
    ...
    //Para agregar tema al modulo ...
    if(isset($_POST['submit'])){
        $idModulo = $_POST['modulo'];
        $nombreTema = $_POST['nombreTema'];
        $query = "INSERT INTO temarioModulos (nombre, idModulo) VALUES ('$nombreTema', $idModulo);";
        $result = mysqli_query($conn, $query);
    }
    ...
    if(isset($_POST['eliminar'])){
        foreach ($_POST['idTemario'] as $idTemario) {
            $query = "DELETE FROM temarioModulos WHERE idTemario = $idTemario;";
    ...
    <div class="container mx-auto p-10">
        ...
        <form action="" method="post">
            <div class="mb-6">
                ...
                <input type="text" name="nombreTema" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" placeholder="A. Bases de la intervención terapeutica en la comunicacion y el lenguaje" required="">
        ...
        <form action="" method="post">
            ...
            <table class="rounded-t-lg m-5 mx-auto bg-gray-200 text-gray-800 w-full">
                ...
                <?php
                    $query = "SELECT idTemario, nombre FROM temarioModulos WHERE idModulo = $idModulo";
                    $result = mysqli_query($conn, $query);
                    while ($temas = mysqli_fetch_array($result)){
                        echo '<tr class="bg-gray-100 border-b border-gray-200">';
                            echo '<td class="px-4 py-5">'.$temas['idTemario'].'</td>';
                            echo '<td class="px-4 py-5">'.$temas['nombre'].'</td>';
                            echo '<td class="px-4 py-5 text-center"><input type="checkbox" name="idTemario[]" value="'.$temas['idTemario'].'"></td>';
                        echo '</tr>';
                    }
                ?>
            </table>
