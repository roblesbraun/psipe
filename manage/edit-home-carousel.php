<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $query = "INSERT INTO carruselHome (rutaImagen) VALUES (1);";
        $result = mysqli_query($conn, $query);
        $query = "SELECT idImagen FROM carruselHome ORDER BY idImagen desc limit 1";
        $result = mysqli_query($conn, $query);
        $idImagen = '';
        ...
        $query = "UPDATE carruselHome SET rutaImagen = '".$idImagen."-".$imagen."' WHERE idImagen = ".$idImagen.";";
        if (!mysqli_query($conn, $query)) {
            die('Error: ' . mysqli_error($conn));
        }
        move_uploaded_file($tmpImagen, '../img/carruselHome/'.$idImagen.'-'.$imagen);
        header("Location: edit-home-carousel.php", true, 303);
    }

    //Para eliminar un archivo de una clase
    if(isset($_POST['eliminar'])){
        foreach ($_POST['idImagen'] as $idImagen) {
            $query= "SELECT rutaImagen FROM carruselHome WHERE idImagen = $idImagen;";
            $result = mysqli_query($conn, $query);
            $rutaImagen = mysqli_fetch_array($result);
            unlink('../img/carruselHome/'.$rutaImagen['rutaImagen'].'');
            $query = "DELETE FROM carruselHome WHERE idImagen = $idImagen;";
...
<head>
    ...
    <title>Agregar/Eliminar Imagenes de Carrusel de Inicio</title>
...
<body>
    ...
    <div class="container mx-auto p-10">
        ...
        <h1 class="text-2xl text-center mb-5">Agregar/Eliminar imagenes de carrusel de inicio</h1>
        ...
        <form action="" method="post">
            ...
            <table class="rounded-t-lg m-5 mx-auto bg-gray-200 text-gray-800 w-full">
                ...
                <?php
                    $queryIm = "SELECT idImagen, rutaImagen FROM carruselHome;";
                    ...
                    if (mysqli_num_rows($resultIm) > 0) {
                        while ($imagen = mysqli_fetch_array($resultIm)){
                            echo '<tr class="bg-gray-100 border-b border-gray-200">';
                                ...
                                echo '<td class="px-4 py-5"><img src="../img/carruselHome/'.$imagen['rutaImagen'].'" class="rounded-lg h-20 w-20"></td>';
